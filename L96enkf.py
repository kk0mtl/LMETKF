...
class L96enkf:
    def __init__(self, nts=2000, enkf_method=3, nEnKF=1, dt_da=0.05, N=40, F=8.0, nem=10, nem_sclfct=1, sp_dist=1, proc_q_covinfl_par=1.0, proc_errstddev=1.0, obs_errstddev=1.0, cov_loc_dist=1.0, cov_loc_dist_pert=None, nobs=100, obs_pos_dst_seed=592, truth_seed=42, obs_pos_dst=1, h_type=0, obs_loc_dist=1.0, row_rank_thresh = 0.99, cov_infl_method=-1, cov_infl_par1=0.4, cov_infl_par2=1.0, K_solver=3, plot_show=False, plot_save=True, plot_data_save=False, mpi=False, loop_test=False):
        ...
        #    - 3:  LETKF 
        #    - 17: GETKF 
        #    - 18: LMETKF (modulated ensemble based mean update and local perturbation update) 
        ...
        if self.plot_show or self.plot_save or self.plot_data_save:
            ...
            if (self.plot_show or self.plot_save) and (not os.path.exists(self.plot_outdir)):
                os.makedirs(self.plot_outdir, exist_ok=True)

            if (self.plot_data_save) and (not os.path.exists(self.plot_data_outdir)): 
                os.makedirs(self.plot_data_outdir, exist_ok=True)
    ...
    def da(self, x, xmean, xptb, obs):
        ...
        elif self.enkf_method == 18: # LMETKF
            return lmetkf(xmean, xptb, self.h, self.h_type, obs, self.obs_r, self.locmtx, self.obs_rloc, self.z, self.mpi, self.comm, self.myrank, self.obs_dist, self.locmtx_pert)
        else:
...
if __name__ == "__main__":
    ...
    def plot_state_mean_timeseries(obj, t_start=3000, t_end=4000):
        ...
        m = (its >= t_start) & (its <= t_end)
        t = its[m]

        x_true_mean = np.mean(x_true[m, :], axis=1)
        x_b_mean = np.mean(x_b[m, :], axis=1)

        plt.figure(figsize=(12, 2.5))

        plt.plot(t, x_true_mean,
            color="black",
            linewidth=1.5,
            label="Truth"
        )
        ...
        if save:
            fname = "state_mean_timeseries.png"
                ...
                for nem in nem_list:
                    for dist in obs_loc_dist:
                        for thresh in row_rank_thresh:
                            # to plot histograms and covariances for figure 4, 5 ->  plot_save=True, plot_data_save=True
